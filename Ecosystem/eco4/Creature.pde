class Creature {
  ...
  //mate if the creature's energy is 130units or more
  void mateC(ArrayList<Creature> creatures){
    if(energy >= 130){
      this.mate(creatures);
    }
  }
  ...
  void reproduction(ArrayList<Creature> creatures){
    Creature c = new Creature();
    Creature partner =  new Creature();
    float record = 130;
   ...
   if(c.energy >= 130){
    for(int i = 0; i < creatures.size(); i++){
      float energyC = ((Creature)creatures.get(i)).energy;
         if(energyC > record){
          energyC = record;
          partner = creatures.get(i);
           }
           
           this.seekForMating(partner);
  ...
  void mate(ArrayList<Creature> creatures){
 
    for(int i = 0; i < creatures.size(); i++){
      float d = PVector.dist(location, creatures.get(i).location);
       if((d > 0) && (d < DNArangeOfSight)){
         this.reproduction(creatures);
       }
